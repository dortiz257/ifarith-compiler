...
(define (ifarith->ifarith-tiny e)
  (match e
    ...
    [`(let* ([,(? symbol? x) ,e]) ,e-body)
     `(let ([,x ,((ifarith->ifarith-tiny e))] ,(ifarith->ifarith-tiny e-body)))]
    [`(let* ([,(? symbol? x) ,e] ,rest-binding-pairs ...) ,e-body)
     `(let ([,x ,(ifarith->ifarith-tiny e)]) ,(ifarith->ifarith-tiny `(let* (,@rest-binding-pairs) ,e-body)))]
    ;; print an arbitrary expression (must be a number at runtime)
    [`(print ,_) e]
    ;; and/or, with short-circuiting semantics
